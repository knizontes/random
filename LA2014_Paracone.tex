...
\subsection{Le Pool}\label{pool}
 ...
 aver effettuato una crittanalisi di SHA-1 che gli consenta di prevedere futuri
...
\end{enumerate}
 ...
 \subsection{La Stima dell'Entropia}
...
\begin{enumerate}
  ...
  durante i trasferimenti di entropia da e verso le pool.
\end{enumerate}
...
\subsubsection{Assunzioni e vincoli per l'\emph{entropy estimator}}
Prima di analizzare com'è implementato l'\emph{entropy estimator}, diamo
ragione brevemente delle assunzioni e dei vincoli sugli \emph{entropy
...
\end{itemize}
 ...
 \subsubsection{La stima dell'entropia per \emph{entropy input}}
 A seconda delle sorgenti di entropia cambia il modo di stimare
 l'impredicibilità introdotta dallo specifico \emph{entropy input}.
 ...
 retroazine lineare} (\emph{linear feedback shift register} o \emph{LFSR})
...
ma si distingue da essa per alcuni dettagli funzionali e implementativi:
 ...
 In questa sezione diamo una breve visione di come sono implementati i requisiti
 di sicurezza nel LRNG, mentre per il lettore alla ricerca di una
 ...
 In \emph{``The Linux Pseudorandom Number Generator Revisited''} (estendendo 
 quanto trattato da \emph{``Analysis of the Linux Random Number Generator''})
 vengono elencati i seguenti requisiti di sicurezza:
 \begin{itemize}
   \item buona stima di entropia: l'entropia introdotta deve essere stimata
   in modo da rendere difficile per un attaccante prevedere gli output senza
   conoscere gli input;
   \item 
 \end{itemize}
